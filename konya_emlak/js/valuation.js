...
function renderStep1() {
  ...
  return `
    ...
    <div class="val-actions" style="margin-top:24px;">
      <button class="val-next" onclick="if(VAL.category){goStep(2);}else{if(typeof showError==='function')showError('Lütfen bir kategori seçin.','Eksik');}" ${!VAL.category ? 'disabled' : ''}>Devam Et →</button>
    </div>`;
}
...
function renderStep3() {
  ...
  return `
    ...
    <div class="vfield">
      ...
      <div class="vchips">
        ${heatings.map(h => `<button class="vchip ${VAL.heating===h?'on':''}" onclick="VAL.heating='${h}';document.querySelectorAll('.vheat').forEach(x=>x.classList.remove('on'));this.classList.add('on','vheat');" class="vchip vheat">${h}</button>`).join('')}
      </div>
    </div>
    <div class="vfield">
      <label class="vlbl">YAPI DURUMU</label>
      <div class="vchips">
        ${['Bakımlı/Yenilenmiş','Standart','Tadilat İhtiyacı Var'].map(c => `<button class="vchip ${VAL.condition===c?'on':''}" onclick="VAL.condition='${c}';document.querySelectorAll('.vcond').forEach(x=>x.classList.remove('on'));this.classList.add('on','vcond');">${c}</button>`).join('')}
      </div>
    </div>
    <div class="val-actions">
